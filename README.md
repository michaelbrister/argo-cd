...
`apps/`
: Child applications managed by the root app. This includes platform apps like Argo CD, Sealed Secrets, `AppProject` definitions, and `ApplicationSet` definitions.

`platform/projects`
: Shared `AppProject` resources for platform-owned and workload-owned applications.

`platform/appsets`
: `ApplicationSet` resources that generate repeated workload applications.

`platform/workloads`
: Example workload directories consumed by the sample Git directory generator.
...
- `projects.yaml`: an `Application` that applies `AppProject` resources from `platform/projects`
- `appsets.yaml`: an `Application` that applies `ApplicationSet` resources from `platform/appsets`

This means Argo CD becomes self-managed after the initial Helm install, Sealed Secrets moves under GitOps management after its one-time bootstrap install, and repeated workload applications can be generated by `ApplicationSet` instead of being hand-authored one by one.

The bootstrap exception is intentional: [apps/projects.yaml](/Users/mike/Documents/src/argo-cd/apps/projects.yaml) still uses the `default` project so it can create the `platform` and `workloads` projects first. The other platform apps then run under the `platform` project instead of staying on `default`.
...
- `projects` and `appsets` appear after `root-app` syncs
- generated workload apps appear after `appsets` syncs
- the platform apps eventually move to `Synced` and `Healthy`
...
- App-of-Apps for platform bootstrap via [bootstrap/root-application.yaml](/Users/mike/Documents/src/argo-cd/bootstrap/root-application.yaml)
- ApplicationSet for repeated workloads via [platform/appsets/workloads-git-generator.yaml](/Users/mike/Documents/src/argo-cd/platform/appsets/workloads-git-generator.yaml)
- a read-only deploy key as the bootstrap credential

That is a practical hybrid model: explicit platform bootstrap, generated workload applications, and encrypted secrets in Git.

## Hybrid layout

This repository now demonstrates a common hybrid pattern:

- App-of-Apps for the bootstrap and platform layer
- `ApplicationSet` for repeated workload generation

The ownership split is:

- `apps/`: explicit platform applications owned by the platform team
- `platform/projects/`: guardrails that define what platform and workload apps may deploy
- `platform/appsets/`: workload generators
- `platform/workloads/`: per-environment workload directories that the generator turns into Argo CD `Application` resources

Project usage in this starter is:

- `default`: bootstrap only, used by [apps/projects.yaml](/Users/mike/Documents/src/argo-cd/apps/projects.yaml)
- `platform`: platform-managed applications such as Argo CD, Sealed Secrets, and ApplicationSets
- `workloads`: generated workload applications

The included example generator in [platform/appsets/workloads-git-generator.yaml](/Users/mike/Documents/src/argo-cd/platform/appsets/workloads-git-generator.yaml) scans directories matching `platform/workloads/*/*`. The sample workload at [platform/workloads/dev/guestbook/kustomization.yaml](/Users/mike/Documents/src/argo-cd/platform/workloads/dev/guestbook/kustomization.yaml) becomes an Argo CD application named `dev-guestbook` in the `workloads` project.
...
- tighten the `platform` and `workloads` `AppProject` policies further so they grant only the repositories, destinations, and resource scopes you actually need
...
2. Refine the example `AppProject` resources into stricter environment- and team-specific policy boundaries.
